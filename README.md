...
`config.json` is a single multi-host file shared across both Macs (it
syncs cleanly via Syncthing because it isn't host-specific). At startup
the server picks the local host's block by matching `os.hostname()`
against `host_aliases`, then merges that block over the `common`
defaults.

```json
{
  "common":        { "port": 3456, "shell": { ... }, "plex": { ... } },
  "host_aliases":  { "Mees-Notebook.local": "mac-notebook", ... },
  "hosts": {
    "mac-notebook": { "priority": 5,  "wireguard_ip": "...", "api_key": "...", "tls": { ... } },
    "mac-studio":   { "priority": 10, "wireguard_ip": "...", "api_key": "...", "tls": { ... } }
  }
}
```
...
| `tls.cert` / `tls.key` | Per-host server cert + key (generated by `scripts/gen-certs.sh`) |
| `spotify`, `messages`, ... | Override common defaults for this host |

Common fields (`port`, `shell.allowlist`, `plex`, etc.) live under
`common` so the same value applies to every host.

The host is resolved in this order: `MCP_HOST` env var → `host_aliases[os.hostname()]`
→ `host_aliases[<hostname-without-.local>]` → literal `os.hostname()`.
Set `MCP_HOST` (e.g. in the launchd plist) if a Mac's system hostname
isn't covered by `host_aliases`.

Legacy single-host configs (no `hosts` key, fields at top level) are
still loaded for backwards compatibility, but new hosts should use the
multi-host shape.
...
Update the `wireguard_ip` for each host's block under `hosts.*` in
